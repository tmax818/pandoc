...
\href{https://ctan.org/pkg/multirow}{\texttt{multirow}} (if the document
contains a table with cells that cross multiple rows),
\href{https://ctan.org/pkg/graphicx}{\texttt{graphicx}} (if the document
contains images),
\href{https://ctan.org/pkg/bookmark}{\texttt{bookmark}},
...
\href{https://ctan.org/pkg/xecjk}{\texttt{xeCJK}} is needed if
\texttt{xelatex} is used, else
\href{https://ctan.org/pkg/luatexja}{\texttt{luatexja}} is needed if
\texttt{lualatex} is used.
\href{https://ctan.org/pkg/framed}{\texttt{framed}} is required if code
is highlighted in a scheme that use a colored background. The use of
\texttt{xelatex} or \texttt{lualatex} as the PDF engine requires
\href{https://ctan.org/pkg/fontspec}{\texttt{fontspec}}.
\texttt{lualatex} uses
\href{https://ctan.org/pkg/selnolig}{\texttt{selnolig}} and
\href{https://ctan.org/pkg/lua-ul}{\texttt{lua-ul}}. \texttt{xelatex}
uses \href{https://ctan.org/pkg/bidi}{\texttt{bidi}} (with the
\texttt{dir} variable set). If the \texttt{mathspec} variable is set,
...
\href{https://ctan.org/pkg/csquotes}{\texttt{csquotes}} package will be
used for \protect\hyperlink{typography}{typography} if the
\texttt{csquotes} variable or metadata field is set to a true value. The
...
rendering}. If math with \texttt{\textbackslash{}cancel},
\texttt{\textbackslash{}bcancel}, or \texttt{\textbackslash{}xcancel} is
used, the \href{https://ctan.org/pkg/cancel}{\texttt{cancel}} package is
...
\tightlist
\item
  \texttt{asciidoc} (\href{https://asciidoc.org/}{AsciiDoc} markup)
\item
...
\item
  ...
  bibliography)
\item
  \texttt{bits} (\href{https://jats.nlm.nih.gov/extensions/bits/}{BITS}
  XML, alias for \texttt{jats})
\item
...
\item
  ...
  table)
\item
  \texttt{djot} (\href{https://djot.net}{Djot markup})
\item
...
\item
  \texttt{mdoc} (\href{https://mandoc.bsd.lv/man/mdoc.7.html}{mdoc}
  manual page markup)
\item
...
\item
  ...
  document})
\item
  \texttt{opml} (\href{https://opml.org/spec2.opml}{OPML})
\item
  \texttt{org} (\href{https://orgmode.org}{Emacs Org mode})
\item
  \texttt{pod} (Perl's \href{https://perldoc.perl.org/perlpod}{Plain Old
  Documentation})
\item
  \texttt{pptx}
...
\item
  ...
  spreadsheet})
\item
  \texttt{xml} (XML version of native AST)
\item
...
\tightlist
\item
  \texttt{ansi} (text with
  \href{https://en.wikipedia.org/wiki/ANSI_escape_code}{ANSI escape
  codes}, for terminal viewing)
\item
  \texttt{asciidoc} (modern \href{https://asciidoc.org/}{AsciiDoc} as
  interpreted by \href{https://asciidoctor.org/}{AsciiDoctor})
\item
  \texttt{asciidoc\_legacy} (\href{https://asciidoc.org/}{AsciiDoc} as
  interpreted by
  \href{https://github.com/asciidoc-py/asciidoc-py}{\texttt{asciidoc-py}}).
\item
  \texttt{asciidoctor} (deprecated synonym for \texttt{asciidoc})
\item
  \texttt{bbcode} \href{https://www.bbcode.org/reference.php}{BBCode}
\item
  \texttt{bbcode\_fluxbb}
  \href{https://web.archive.org/web/20210623155046/https://fluxbb.org/forums/help.php\#bbcode}{BBCode
  (FluxBB)}
\item
  \texttt{bbcode\_phpbb}
  \href{https://www.phpbb.com/community/help/bbcode}{BBCode (phpBB)}
\item
  \texttt{bbcode\_steam}
  \href{https://steamcommunity.com/comment/ForumTopic/formattinghelp}{BBCode
  (Steam)}
\item
  \texttt{bbcode\_hubzilla}
  \href{https://hubzilla.org/help/member/bbcode}{BBCode (Hubzilla)}
\item
  \texttt{bbcode\_xenforo}
...
\item
  \texttt{vimdoc}
  (\href{https://vimhelp.org/helphelp.txt.html\#help-writing}{Vimdoc})
\item
  \texttt{xml} (XML version of native AST)
\item
  \texttt{xwiki}
...
Specify a set of default option settings. \emph{FILE} is a YAML or JSON
...
working directory, and then in the \texttt{defaults} subdirectory of the
user data directory (see \texttt{-\/-data-dir}). The \texttt{.yaml}
extension will be added if \emph{FILE} lacs an extension. See the
section \protect\hyperlink{defaults-files}{Defaults files} for more
...
\item[\texttt{-\/-fail-if-warnings{[}=true\textbar{}false{]}}]
...
\item[\texttt{-\/-extract-media=}\emph{DIR}\textbar{}\emph{FILE}\texttt{.zip}]
...
If the path given ends in \texttt{.zip}, then instead of creating a
...
template syntax. If the template is not found, pandoc will search for it
in the \texttt{templates} subdirectory of the user data directory (see
\texttt{-\/-data-dir}). If no extension is specified and an
...
output. If this option is not used, a default template appropriate for
the output format will be used (see
\texttt{-D/-\/-print-default-template}).
\item[\texttt{-V} \emph{KEY}{[}\texttt{=}\emph{VAL}{]},
\texttt{-\/-variable=}\emph{KEY}{[}\texttt{=}\emph{VAL}{]}]
Set the template variable \emph{KEY} to the string value \emph{VAL} when
rendering the document in standalone mode. Either \texttt{:} or
\texttt{=} may be used to separate \emph{KEY} from \emph{VAL}. If no
\emph{VAL} is specified, the key will be given the value \texttt{true}.
Structured values (lists, maps) cannot be assigned using this option,
but they can be assigned in the \texttt{variables} section of a
\protect\hyperlink{defaults-files}{defaults file} or using the
\texttt{-\/-variable-json} option. If the variable already has a
\emph{list} value, the value will be added to the list. If it already
has another kind of value, it will be made into a list containing the
previous and the new value. For example,
\texttt{-V\ keyword=Joe\ -V\ author=Sue} makes \texttt{author} contain a
list of strings: \texttt{Joe} and \texttt{Sue}.
\item[\texttt{-\/-variable-json=}\emph{KEY}{[}\texttt{=}:\emph{JSON}{]}]
Set the template variable \emph{KEY} to the value specified by a JSON
string (this may be a boolean, a string, a list, or a mapping; a number
will be treated as a string). For example,
\texttt{-\/-variable-json\ foo=false} will give \texttt{foo} the boolean
false value, while
\texttt{-\/-variable-json\ foo=\textquotesingle{}"false"\textquotesingle{}}
will give it the string value \texttt{"false"}. Either \texttt{:} or
\texttt{=} may be used to separate \emph{KEY} from \emph{VAL}. If the
...
\item[\texttt{-\/-toc{[}=true\textbar{}false{]}},
\texttt{-\/-table-of-contents{[}=true\textbar{}false{]}}]
...
Note that if you are producing a PDF via \texttt{ms} and using (the
default) \texttt{pdfroff} as a \texttt{-\/-pdf-engine}, the table of
...
\texttt{-\/-pdf-engine-opt=-\/-no-toc-relocation}. If \texttt{groff} is
used as the \texttt{-\/-pdf-engine}, the table of contents will always
...
\item[\texttt{-\/-lof{[}=true\textbar{}false{]}},
\texttt{-\/-list-of-figures{[}=true\textbar{}false{]}}]
...
effect unless \texttt{-s/-\/-standalone} is used, and it only has an
effect on \texttt{latex}, \texttt{context}, and \texttt{docx} output.
\item[\texttt{-\/-lot{[}=true\textbar{}false{]}},
\texttt{-\/-list-of-tables{[}=true\textbar{}false{]}}]
Include an automatically generated list of tables (or, in some formats,
an instruction to create one) in the output document. This option has no
effect unless \texttt{-s/-\/-standalone} is used, and it only has an
effect on \texttt{latex}, \texttt{context}, and \texttt{docx} output.
\item[\texttt{-\/-strip-comments{[}=true\textbar{}false{]}}]
...
\item[\texttt{-\/-syntax-highlighting=default\textbar{}none\textbar{}idiomatic\textbar{}}\emph{STYLE}\texttt{\textbar{}}\emph{FILE}]
The method to use for code syntax highlighting. Setting a specific
\emph{STYLE} causes highlighting to be performed with the internal
highlighting engine, using KDE syntax definitions and styles. The
\texttt{idiomatic} method uses a format-specific highlighter if one is
available, or the default style if the target format has no idiomatic
highlighting method. Setting this option to \texttt{none} disables all
syntax highlighting. The \texttt{default} method uses a format-specific
default.
...
Typst's own syntax highlighting system.

Style options are \texttt{pygments} (the default), \texttt{kate},
...
\emph{Deprecated, use \texttt{-\/-syntax-highlighting=none} instead.}

Disables syntax highlighting for code blocks and inlines, even when a
language attribute is given.
\item[\texttt{-\/-highlight-style=}\emph{STYLE}\textbar{}\emph{FILE}]
\emph{Deprecated, use
\texttt{-\/-syntax-highlighting=}\emph{STYLE}\textbar{}\emph{FILE}
instead.}
...
\texttt{-\/-syntax-highlighting}. This option may be used with
\texttt{-o}/\texttt{-\/-output} to redirect output to a file, but
\texttt{-o}/\texttt{-\/-output} must come before
...
\texttt{-\/-standalone}. Note that if the output format is \texttt{odt},
this file must be in OpenDocument XML format suitable for insertion into
the body of the document, and if the output is \texttt{docx}, this file
...
included in the order specified. Implies \texttt{-\/-standalone}. Note
that if the output format is \texttt{odt}, this file must be in
OpenDocument XML format suitable for insertion into the body of the
document, and if the output is \texttt{docx}, this file must be in
...
equivalent to \texttt{-\/-resource-path\ baz:bim:foo:bar}. Note that
this option only has an effect when pandoc itself needs to find an image
(e.g., in producing a PDF or docx, or when \texttt{-\/-embed-resources}
is used.) It will not cause image paths to be rewritten in other cases
...
\item[\texttt{-\/-no-check-certificate{[}=true\textbar{}false{]}}]
...
For SVG images, \texttt{img} tags with \texttt{data:} URIs are used,
unless the image has the class \texttt{inline-svg}, in which case an
inline SVG element is inserted. This approach is recommended when there
are many occurrences of the same SVG in a document, as
\texttt{\textless{}use\textgreater{}} elements will be used to reduce
duplication.
\item[\texttt{-\/-link-images{[}=true\textbar{}false{]}}]
...
\item[\texttt{-\/-figure-caption-position=above}\textbar{}\texttt{below}]
Specify whether figure captions go above or below figures (default is
\texttt{below}). This option only affects HTML, LaTeX, Docx, ODT, and
Typst output.
\item[\texttt{-\/-table-caption-position=above}\textbar{}\texttt{below}]
Specify whether table captions go above or below tables (default is
\texttt{above}). This option only affects HTML, LaTeX, Docx, ODT, and
Typst output.
...
headings if \texttt{chapter} is selected, and before first- and
second-level headings if \texttt{part} is selected. Footnote numbers
will restart with each section break unless the reference doc modifies
this.
\item[\texttt{-N},
\texttt{-\/-number-sections={[}true\textbar{}false{]}}]
...
\texttt{-\/-number-offset=5}. If your document starts with a level-2
heading which you want to be numbered ``1.5'', specify
\texttt{-\/-number-offset=1,4}. \texttt{-\/-number-offset} only directly
affects the number of the first section heading in a document;
subsequent numbers increment in the normal way. Implies
\texttt{-\/-number-sections}. Currently this feature only affects HTML
and Docx output.
\item[\texttt{-\/-listings{[}=true\textbar{}false{]}}]
*Deprecated, use \texttt{-\/-syntax-highlighting=idiomatic} or
\texttt{-\/-syntax-highlighting=default} instead.
...
\texttt{beamer}, \texttt{revealjs}, \texttt{pptx}, \texttt{s5},
\texttt{slidy}, \texttt{slideous}, \texttt{dzslides}). Headings above
...
\item[\protect\hypertarget{option--reference-doc}{}{\texttt{-\/-reference-doc=}\emph{FILE}\textbar{}\emph{URL}}]
...
\texttt{context}, \texttt{groff}, \texttt{pdfroff}, and \texttt{typst}.
...
\item
  \texttt{-t\ html}: \texttt{weasyprint} (other options:
  \texttt{prince}, \texttt{wkhtmltopdf}, \texttt{pagedjs-cli}; see
...
as \texttt{-o/-\/-output}. However, it may still have an effect when
other output formats are requested. For example, \texttt{ms} output will
include \texttt{.pdfhref} macros only if a \texttt{-\/-pdf-engine} is
selected, and the macros will be differently encoded depending on
whether \texttt{groff} or \texttt{pdfroff} is specified.
...
Note: if this option is specified, the \texttt{citations} extension will
...
options, in the form of a YAML or JSON file. Examples in this section
will be given in YAML, but the equivalent forms in JSON will also work.
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{\{} \DataTypeTok{"verbosity"}\FunctionTok{:} \StringTok{"INFO"} \FunctionTok{\}}
\end{Highlighting}
\end{Shaded}
...
\AttributeTok{  }\KeywordTok{{-}}\AttributeTok{ citeproc}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{wrap}\KeywordTok{:}\AttributeTok{ }\StringTok{"preserve"}
\end{Highlighting}
\end{Shaded}
\end{minipage} \\
\begin{minipage}[t]{\linewidth}\raggedright
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{syntax{-}highlighting}\KeywordTok{:}\AttributeTok{ }\StringTok{\textquotesingle{}none\textquotesingle{}}
\end{Highlighting}
\end{Shaded}
\end{minipage} \\
\begin{minipage}[t]{\linewidth}\raggedright
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{syntax{-}highlighting}\KeywordTok{:}\AttributeTok{ kate}
\end{Highlighting}
\end{Shaded}
\end{minipage} \\
\begin{minipage}[t]{\linewidth}\raggedright
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{link{-}images}\KeywordTok{:}\AttributeTok{ }\CharTok{true}
\end{Highlighting}
\end{Shaded}
\end{minipage} \\
\begin{minipage}[t]{\linewidth}\raggedright
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{figure{-}caption{-}position}\KeywordTok{:}\AttributeTok{ above}
\end{Highlighting}
\end{Shaded}
\end{minipage} \\
\begin{minipage}[t]{\linewidth}\raggedright
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{table{-}caption{-}position}\KeywordTok{:}\AttributeTok{ below}
\end{Highlighting}
\end{Shaded}
\end{minipage} \\
\begin{minipage}[t]{\linewidth}\raggedright
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{list{-}of{-}figures}\KeywordTok{:}\AttributeTok{ }\CharTok{true}
\end{Highlighting}
\end{Shaded}
\end{minipage} \\
\begin{minipage}[t]{\linewidth}\raggedright
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{lof}\KeywordTok{:}\AttributeTok{ }\CharTok{true}
\end{Highlighting}
\end{Shaded}
\end{minipage} \\
\begin{minipage}[t]{\linewidth}\raggedright
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{list{-}of{-}tables}\KeywordTok{:}\AttributeTok{ }\CharTok{true}
\end{Highlighting}
\end{Shaded}
\end{minipage} \\
\begin{minipage}[t]{\linewidth}\raggedright
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{lot}\KeywordTok{:}\AttributeTok{ }\CharTok{true}
\end{Highlighting}
\end{Shaded}
\end{minipage} \\
\begin{minipage}[t]{\linewidth}\raggedright
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{bibliography}\KeywordTok{:}\AttributeTok{ logic.bib}
\end{Highlighting}
\end{Shaded}
\end{minipage} \\
\begin{minipage}[t]{\linewidth}\raggedright
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{csl}\KeywordTok{:}\AttributeTok{ ieee.csl}
\end{Highlighting}
\end{Shaded}
\end{minipage} \\
\begin{minipage}[t]{\linewidth}\raggedright
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{citation{-}abbreviations}\KeywordTok{:}\AttributeTok{ ab.json}
\end{Highlighting}
\end{Shaded}
\end{minipage} \\
\begin{minipage}[t]{\linewidth}\raggedright
...
of \texttt{filters} (see \protect\hyperlink{reader-options-1}{Reader
options}).
...
\item
  ...
  template.
\item
  For \texttt{docx} output, customize the \texttt{default.openxml}
  template.
\item
  ...
  \texttt{-t\ html}).
\item
  \texttt{pptx} has no template.
\end{itemize}

Note that \texttt{docx}, \texttt{odt}, and \texttt{pptx} output can also
be customized using \texttt{-\/-reference-doc}. Use a reference doc to
adjust the styles in your document; use a template to handle variable
interpolation and customize the presentation of metadata, the position
of the table of contents, boilerplate text, etc.
...
\texttt{variable} has a true value, otherwise the \texttt{else} section
is used (if present). The following values count as true:
...
using \texttt{-M/-\/-metadata}), unquoted \texttt{true} and
\texttt{false} will be interpreted as Boolean values. But a variable
specified on the command line using \texttt{-V/-\/-variable} will always
be given a string value. Hence a conditional \texttt{if(foo)} will be
triggered if you use \texttt{-V\ foo=false}, but not if you use
\texttt{-M\ foo=false}.
...
Unless you use \texttt{-\/-css}, this variable is set to \texttt{true}
...
\item[\texttt{maxwidth}]
sets the CSS \texttt{max-width} property (default is 36em).
...
when using \texttt{-\/-katex}, you can render display math equations
flush left using \protect\hyperlink{layout}{YAML metadata} or with
...
\texttt{https://unpkg.com/reveal.js@\^{}5})
\item[\texttt{s5-url}]
...
\item[\texttt{highlightjs-theme}]
highlight.js theme for code highlighting when using
\texttt{-\/-syntax-highlighting=idiomatic} with reveal.js (defaults to
\texttt{monokai}). See the
\href{https://highlightjs.org/demo}{highlight.js demo page} for
available themes.
\end{description}
...
\item[\texttt{logooptions}]
options for logo image (e.g., \texttt{width}, \texttt{height})
...
beamer themes
\item[\texttt{themeoptions}, \texttt{colorthemeoptions},
\texttt{fontthemeoptions}, \texttt{innerthemeoptions},
\texttt{outerthemeoptions}]
options for LaTeX beamer themes (lists)
\item[\texttt{titlegraphic}]
image for title slide: can be a list
\item[\texttt{titlegraphicoptions}]
options for title slide image (e.g., \texttt{width}, \texttt{height})
\item[\texttt{shorttitle}, \texttt{shortsubtitle}, \texttt{shortauthor},
\texttt{shortinstitute}, \texttt{shortdate}]
...
...
\end{verbatim}
\item[\texttt{shorthands}]
Enable language-specific shorthands when loading \texttt{babel}. (By
default, pandoc includes \texttt{shorthands=off} when loading
\texttt{babel}, disabling language-specific shorthands.)
...
you must specify the beamer writer but use the default \emph{LaTeX}
template: for example,
\texttt{pandoc\ -Vbeamerarticle\ -t\ beamer\ -\/-template\ default.latex}.
\item[\texttt{handout}]
produce a handout version of Beamer slides (with overlays condensed into
single slides)
\item[\texttt{csquotes}]
load \texttt{csquotes} package and use \texttt{\textbackslash{}enquote}
or \texttt{\textbackslash{}enquote*} for quoted text.
\item[\texttt{csquotesoptions}]
options to use for \texttt{csquotes} package (repeat for multiple
options).
\item[\texttt{babeloptions}]
options to pass to the babel package (may be repeated for multiple
options). This defaults to \texttt{provide=*} if the main language isn't
a European language written with Latin or Cyrillic script or Vietnamese.
...
\href{https://ctan.org/pkg/xecjk}{\texttt{xecjk}} package if
\texttt{xelatex} is used, or the
\href{https://ctan.org/pkg/luatexja}{\texttt{luatexja}} package if
\texttt{lualatex} is used.
\item[\texttt{mainfontoptions}, \texttt{sansfontoptions},
\texttt{monofontoptions}, \texttt{mathfontoptions}, \texttt{CJKoptions},
\texttt{luatexjapresetoptions}]
...
...
\end{verbatim}
\item[\texttt{mainfontfallback}, \texttt{sansfontfallback},
\texttt{monofontfallback}]
fonts to try if a glyph isn't found in \texttt{mainfont},
\texttt{sansfont}, or \texttt{monofont} respectively. These are lists.
...
Font fallbacks currently only work with \texttt{lualatex}.
...
\texttt{-\/-lof/-\/-list-of-figures},
\texttt{-\/-lot/-\/-list-of-tables})
\item[\texttt{thanks}]
...
\begin{description}
\item[\texttt{pdf-trailer-id}]
the PDF trailer ID; must be two PDF byte strings if set, conventionally
with 16 bytes each. E.g.,
\texttt{\textless{}00112233445566778899aabbccddeeff\textgreater{}\ \textless{}00112233445566778899aabbccddeeff\textgreater{}}.

See the section on \protect\hyperlink{reproducible-builds}{reproducible
builds}.
\item[\texttt{pdfstandard}]
PDF standard(s) for the document, e.g.~\texttt{ua-2}, \texttt{a-4f}.
Supports PDF/A, PDF/X, and PDF/UA variants. Requires LuaLaTeX and LaTeX
...
font. Use \texttt{\textbackslash{}definefallbackfamily}-compatible font
...
\hypertarget{variables-for-texinfo}{%
\subsubsection{Variables for Texinfo}\label{variables-for-texinfo}}
...
\item[\texttt{page-numbering}]
Schema to use for numbering pages, e.g.~\texttt{1} or \texttt{i}, or an
empty string to omit page numbering.
\item[\texttt{columns}]
Number of columns for body text.
\item[\texttt{thanks}]
contents of acknowledgments footnote after document title
\item[\texttt{mathfont}, \texttt{codefont}]
Name of system font to use for math and code, respectively.
\item[\texttt{linestretch}]
adjusts line spacing, e.g.~\texttt{1.25}, \texttt{1.5}
\item[\texttt{linkcolor}, \texttt{filecolor}, \texttt{citecolor}]
...
\item[\texttt{pdf-engine}]
name of PDF engine if provided using \texttt{-\/-pdf-engine}, or the
...
pptx a custom \texttt{toc-title} will be picked up from metadata, but
...
\texttt{html}
\item[output formats]
\texttt{markdown}, \texttt{latex}, \texttt{context}, \texttt{org},
\texttt{rst}
...
\texttt{heading-identifiers-in-html} \\
\texttt{MaÃ®tre\ d\textquotesingle{}hÃ´tel} &
\texttt{maÃ®tre-dhÃ´tel} \\
...
\protect\hyperlink{extension-tex_math_gfm}{\texttt{tex\_math\_gfm}},
...
When converting from docx, add \texttt{custom-styles} attributes for all
docx styles, regardless of whether pandoc understands the meanings of
these styles. Because attributes cannot be added directly to paragraphs
or text in the pandoc AST, paragraph styles will cause Divs to be
created and character styles will cause Spans to be created to hold the
attributes. (Table styles will be added to the Table elements directly.)
This extension can be used with \protect\hyperlink{custom-styles}{docx
custom styles}.
...
\hypertarget{typst-citations}{%
\subsubsection{\texorpdfstring{Extension: \texttt{citations}
(typst)}{Extension: citations (typst)}}\label{typst-citations}}

When the \texttt{citations} extension is enabled in \texttt{typst} (as
it is by default), \texttt{typst} citations will be parsed as native
pandoc citations, and native pandoc citations will be rendered as
\texttt{typst} citations.

\hypertarget{org-citations}{%
\subsubsection{\texorpdfstring{Extension: \texttt{citations}
(org)}{Extension: citations (org)}}\label{org-citations}}
...
\hypertarget{docx-citations}{%
\subsubsection{\texorpdfstring{Extension: \texttt{citations}
(docx)}{Extension: citations (docx)}}\label{docx-citations}}
...
\subsubsection{\texorpdfstring{Extension: \texttt{fancy\_lists}
(org)}{Extension: fancy\_lists (org)}}\label{org-fancy-lists}}
...
\hypertarget{extension-smart_quotes-org}{%
\subsubsection{\texorpdfstring{Extension: \texttt{smart\_quotes}
(org)}{Extension: smart\_quotes (org)}}\label{extension-smart_quotes-org}}

Interpret straight quotes as curly quotes during parsing. When
\emph{writing} Org, then the \texttt{smart\_quotes} extension has the
reverse effect: what would have been curly quotes comes out straight.

This extension is implied if \texttt{smart} is enabled.

\hypertarget{extension-special_strings-org}{%
\subsubsection{\texorpdfstring{Extension: \texttt{special\_strings}
(org)}{Extension: special\_strings (org)}}\label{extension-special_strings-org}}

Interpret \texttt{-\/-\/-} as em-dashes, \texttt{-\/-} as en-dashes,
\texttt{\textbackslash{}-} as shy hyphen, and \texttt{...} as ellipses.
...
\texttt{-\/-syntax-highlighting=none} option. To set the highlighting
style or method, use \texttt{-\/-syntax-highlighting}. For more
...
lists always behave as if the \texttt{four\_space\_rule} extension is
...
\hypertarget{extension-table_attributes}{%
\subsubsection{\texorpdfstring{Extension:
\texttt{table\_attributes}}{Extension: table\_attributes}}\label{extension-table_attributes}}

Attributes may be attached to tables by including them at the end of the
caption. (For the syntax, see
...
at the beginning, it must be preceded by a blank line. (Note that JSON
may be used as well, because JSON is a subset of YAML.)
...
\subsubsection{\texorpdfstring{Extension: \texttt{superscript},
...
\texttt{\textless{}pre\textgreater{}}, and
...
\texttt{link\_attributes} extension is set):
...
positioning} by adding the \texttt{latex-placement} attribute.
...
newlines, or the characters \texttt{\^{}}, \texttt{{[}}, or
\texttt{{]}}. These identifiers are used only to correlate the footnote
...
\texttt{volume}, \texttt{vol.}/\texttt{vols.};
\texttt{Â¶}/\texttt{Â¶Â¶}; \texttt{Â§}/\texttt{Â§Â§}. If no locator term
...
chapter. To do this, add the class \texttt{reset-citation-positions} to
the heading for each chapter:
...
\hypertarget{extension-tex_math_gfm}{%
\subsubsection{\texorpdfstring{Extension:
\texttt{tex\_math\_gfm}}{Extension: tex\_math\_gfm}}\label{extension-tex_math_gfm}}

Supports two GitHub-specific formats for math. Inline math:
\texttt{\$\textasciigrave{}e=mc\^{}2\textasciigrave{}\$}.
...
\hypertarget{extension-alerts}{%
\subsubsection{\texorpdfstring{Extension:
\texttt{alerts}}{Extension: alerts}}\label{extension-alerts}}
...
with id \texttt{refs}, if one exists:\footnote{Note that if
  \texttt{-\/-file-scope} is used, a div written this way will be given
  an identifier of the form \texttt{FILE\_\_refs}, to avoid duplicate
  identifiers (see \texttt{-\/-file-scope}). In view of this
  possibility, pandoc will place the bibliography in any div whose
  identifier is \texttt{refs} \emph{or} ends with \texttt{\_\_refs}.}
...
\item
  ...
  \texttt{cotÃ©} sorting after \texttt{cÃ´te}).
\item
...
\item[Section Header]
This layout is used for what pandoc calls â€œtitle slidesâ€, i.e. slides
...
\texttt{column}.
\item[Comparison]
This layout is used instead of â€œTwo Contentâ€ for any two-column
...
\hypertarget{speaker-notes-on-the-title-slide-powerpoint}{%
\subsubsection{Speaker notes on the title slide
(PowerPoint)}\label{speaker-notes-on-the-title-slide-powerpoint}}

For PowerPoint output, the title slide is generated from the document's
YAML metadata block. To add speaker notes to this slide, use a
\texttt{notes} field in the metadata:
...
The \texttt{notes} field can contain multiple paragraphs and Markdown
formatting.
...
For pptx, you can use a \texttt{-\/-reference-doc} in which background
images have been set on the
...
consistency with reveal.js â€`` if \texttt{background-image} isnâ€™t
found, \texttt{data-background-image} will be checked.
...
(Note that the \texttt{data-} prefix is required here, as it isnâ€™t
added automatically.)

For pptx, pass a \texttt{-\/-reference-doc} with the background image
set on the â€œTitle Slideâ€ layout.
...
the \texttt{-\/-epub-metadata} option, which takes as its argument an
XML file with
\href{https://www.dublincore.org/specifications/dublin-core/dces/}{Dublin
Core elements}.
...
\protect\hyperlink{extension-yaml_metadata_block}{YAML metadata block}
in a Markdown document, or in a separate YAML file specified with
\texttt{-\/-metadata-file}. Here is an example of a YAML metadata block
with EPUB metadata:
...
\texttt{Legal\ deposit\ number}, \texttt{URN}, \texttt{OCLC\ number},
\texttt{Co-publisherâ€™s\ ISBN-13}, \texttt{ISMN-13}, \texttt{ISBN-A},
\texttt{JP\ e-code}, \texttt{OLCC\ number}, \texttt{JP\ Magazine\ ID},
\texttt{UPC-12+5}, \texttt{BNF\ Control\ number}, \texttt{ISSN-13},
\texttt{ARK}, \texttt{Digital\ file\ internal\ version\ number}.
\item[\texttt{title}]
...
element}.
\item[\texttt{accessModes}]
An array of strings
(\href{https://kb.daisy.org/publishing/docs/metadata/schema.org/index.html}{schema}).
Defaults to \texttt{{[}"textual"{]}}.
\item[\texttt{accessModeSufficient}]
An array of strings
(\href{https://kb.daisy.org/publishing/docs/metadata/schema.org/index.html}{schema}).
Defaults to \texttt{{[}"textual"{]}}.
\item[\texttt{accessibilityHazards}]
An array of strings
(\href{https://kb.daisy.org/publishing/docs/metadata/schema.org/index.html}{schema}).
Defaults to \texttt{{[}"none"{]}}.
\item[\texttt{accessibilityFeatures}]
An array of strings
(\href{https://kb.daisy.org/publishing/docs/metadata/schema.org/index.html}{schema}).
Defaults to
...
- "tableOfContents"
\end{verbatim}
\item[\texttt{accessibilitySummary}]
A string value.
\item[\texttt{ibooks}]
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{abstract}\KeywordTok{:}\AttributeTok{ }\StringTok{"A short description"}
\FunctionTok{author}\KeywordTok{:}\AttributeTok{ Author}
\FunctionTok{title}\KeywordTok{:}\AttributeTok{ Title}

\CommentTok{\# Vimdoc{-}specific}
\FunctionTok{filename}\KeywordTok{:}\AttributeTok{ }\StringTok{"definition{-}lists.txt"}
\FunctionTok{vimdoc{-}prefix}\KeywordTok{:}\AttributeTok{ pandoc}
\end{Highlighting}
\end{Shaded}

Complete header requires \texttt{abstract}, \texttt{author},
\texttt{title} and \texttt{filename} to be set. Compiling file with such
metadata produces the following file (assumes \texttt{-\/-standalone},
see \protect\hyperlink{templates}{Templates}):

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{*definition{-}lists.txt*  A short description}

\NormalTok{                            Title by Author}


\NormalTok{                                 Type |gO| to see the table of contents.}

\NormalTok{[...]}

\NormalTok{ vim:tw=72:sw=4:ts=4:ft=help:norl:et:}
\end{Highlighting}
\end{Shaded}

If \texttt{vimdoc-prefix} is set, all non-command tags are prefixed with
its value, it is used to prevent tag collision: all headers have a tag
...
\InformationTok{\textasciigrave{}:[range]Fnl \{expr\}\textasciigrave{}}\NormalTok{\{\#:Fnl\}}
\NormalTok{:   Evaluates \{expr\} or range}

\InformationTok{\textasciigrave{}vim.b\textasciigrave{}}\NormalTok{\{\#vim.b\}}
\NormalTok{:   Buffer{-}scoped (}\InformationTok{\textasciigrave{}:h b:\textasciigrave{}}\NormalTok{) variables for the current buffer. Invalid or unset}
\NormalTok{    key returns }\InformationTok{\textasciigrave{}nil\textasciigrave{}}\NormalTok{. Can be indexed with an integer to access variables for a}
\NormalTok{    specific buffer.}

\CommentTok{[}\OtherTok{Span}\CommentTok{]}\NormalTok{\{\#span\}}
\NormalTok{:   generic inline container for phrasing content, which does not inherently}
\NormalTok{    represent anything.}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\NormalTok{{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}}
\NormalTok{Header                                                            *header*}

\NormalTok{:[range]Fnl \{expr\}                                                  *:Fnl*}
\NormalTok{    Evaluates \{expr\} or range}
\NormalTok{\textasciigrave{}vim.b\textasciigrave{}                                                            *vim.b*}
\NormalTok{    Buffer{-}scoped (|b:|) variables for the current buffer. Invalid or}
\NormalTok{    unset key returns \textasciigrave{}nil\textasciigrave{}. Can be indexed with an integer to access}
\NormalTok{    variables for a specific buffer.}
\NormalTok{Span                                                                *span*}
\NormalTok{    generic inline container for phrasing content, which does not}
\NormalTok{    inherently represent anything.}
\end{Highlighting}
\end{Shaded}

Convert it to vimdoc with metadata variable set (e.g.Â with
\texttt{-M\ vimdoc-prefix=pandoc})

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}{-}}
\NormalTok{Header                                                     *pandoc{-}header*}

\NormalTok{:[range]Fnl \{expr\}                                                  *:Fnl*}
\NormalTok{    Evaluates \{expr\} or range}
\NormalTok{\textasciigrave{}vim.b\textasciigrave{}                                                     *pandoc{-}vim.b*}
\NormalTok{    Buffer{-}scoped (|b:|) variables for the current buffer. Invalid or}
\NormalTok{    unset key returns \textasciigrave{}nil\textasciigrave{}. Can be indexed with an integer to access}
\NormalTok{    variables for a specific buffer.}
\NormalTok{Span                                                         *pandoc{-}span*}
\NormalTok{    generic inline container for phrasing content, which does not}
\NormalTok{    inherently represent anything.}
\end{Highlighting}
\end{Shaded}

\texttt{vim.b} and \texttt{Span} got their prefixes but not
\texttt{:Fnl} because ex-commands (those starting with \texttt{:}) don't
get a prefix, since they are considered unique across help pages.

In both cases \texttt{:help\ b:} became reference
\texttt{\textbar{}b:\textbar{}} (also works with \texttt{:h\ b:}). Links
pointing to either \url{https://vimhelp.org/} or
\url{https://neovim.io/doc/user} become references too.

Vim traditionally wraps at 78, but Pandoc defaults to 72. Use
\texttt{-\/-columns\ 78} to match Vim.
...
\texttt{-\/-syntax-highlighting} option. The default color scheme is
\texttt{pygments}, which imitates the default color scheme used by the
Python library pygments (though pygments is not actually used to do the
highlighting). To see a list of highlight styles, type
\texttt{pandoc\ -\/-list-highlight-styles}.
...
\texttt{-\/-syntax-highlighting}. To get a JSON version of the
...
To disable highlighting, use \texttt{-\/-syntax-highlighting=none}.

To use a format's idiomatic syntax highlighting instead of pandoc's
built-in highlighting, use \texttt{-\/-syntax-highlighting=idiomatic}.
Currently, \texttt{idiomatic} only affects the following formats:

\begin{itemize}
\item
  In reveal.js, it causes reveal.js's highlighting plugin to be used for
  source code highlighting. The style may be customized by setting the
  \texttt{highlightjs-theme} variable.
\item
  In Typst, it causes Typst's built-in highlighting to be used. (This is
  also the default for Typst.)
\item
  In LaTeX, it causes the
  \href{https://ctan.org/pkg/listings}{\texttt{listings}} package to be
  used. Note that \texttt{listings} does not support multi-byte encoding
  for source code. To handle UTF-8 you would need to use a custom
  template. This issue is fully documented here:
  \href{https://en.wikibooks.org/wiki/LaTeX/Source_Code_Listings\#Encoding_issue}{Encoding
  issue with the listings package}.
\item
  In other formats, \texttt{idiomatic} will have the same result as
  \texttt{default}.
\end{itemize}
...
\protect\hyperlink{option--reference-doc}{reference doc} file. However,
...
your \protect\hyperlink{option--reference-doc}{reference doc}. If they
are already defined, pandoc will not alter the definition.
...
For icml output, you can also set an \texttt{object-style} in images:
...
the \texttt{custom-style} class. A \texttt{custom-style} attribute will
be added for each style. Divs will be created to hold the paragraph
styles, and Spans to hold the character styles. Table styles will be
...
\texttt{pdf-trailer-id} in the metadata or leave it undefined, in which
case pandoc will create a trailer-id based on a hash of the
\texttt{SOURCE\_DATE\_EPOCH} and the document's contents.
...
\texttt{\textbackslash{}DocumentMetadata} interface supports PDF
standards and tagging when using LuaLaTeX; set the \texttt{pdfstandard}
variable to enable this (see below). For older LaTeX installations,
alternative engines must be used.
...
Set the \texttt{pdfstandard} variable to produce tagged PDFs conforming
...
and
\href{https://help.libreoffice.org/latest/en-US/text/shared/01/ref_pdf_export_general.html}{LibreOffice}.
...
\texttt{lua} executable}, version 5.4. All \texttt{pandoc.*} packages,
as well as the packages \texttt{re} and \texttt{lpeg}, are available via
global variables. Furthermore, the globals \texttt{PANDOC\_VERSION},
\texttt{PANDOC\_STATE}, and \texttt{PANDOC\_API\_VERSION} are set at
startup. For full documentation, see the
...
\item
  ...
  \texttt{iframe} elements by fetching content from the local file or
  URL specified by \texttt{src}. If untrusted HTML is processed on a
  server, this has the potential to reveal anything readable by the
  process running the server. Using the \texttt{-f\ html+raw\_html} will
  mitigate this threat by causing the whole \texttt{iframe} to be parsed
  as a raw HTML block. Using \texttt{-\/-sandbox} will also protect
  against the threat.
\item
